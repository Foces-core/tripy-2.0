...
function BrandIcon({ d, label }: { d: string; label: string }) {
  return (
    <svg
      role="img"
      aria-label={label}
      viewBox="0 0 24 24"
      className="h-4 w-4 fill-[#d8a84e]"
      aria-hidden={false}
    >
      <title>{label}</title>
      <path d={d} />
  ...
  return (
    <footer className="mt-6 flex items-center justify-center gap-4 border-t border-[#f4e8c6]/10 pt-3 text-xs tracking-widest">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label="FOCES CEC on LinkedIn"
        className="opacity-80"
      >
        <BrandIcon d={LINKEDIN_PATH} label="LinkedIn" />
      </a>
      <a
        href="https://www.instagram.com/foces_cec"
        target="_blank"
        rel="noreferrer"
        aria-label="FOCES CEC on Instagram"
        className="opacity-80"
      >
        <BrandIcon d={INSTAGRAM_PATH} label="Instagram" />
      </a>
    </footer>
  );
}
